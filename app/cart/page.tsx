      ...
      {cartItems.length === 0 ? (
        ...
        <>
            ...
            {cartItems.map((item) => (
              <div key={item.id} className="flex items-center space-x-4 bg-white p-4 rounded-lg shadow">
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-amber-600">دج{item.price.toFixed(2)}</p>
                </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-2xl font-bold mb-4">طلبيتك</h2>
            <p className="text-xl mb-4">
              Total: دج{cartItems.reduce((sum, item) => sum + item.price * item.quantity, 0).toFixed(2)}
            </p>
            <form onSubmit={submitOrder} className="space-y-4">
              <div>
                <label htmlFor="name" className="block mb-1">الاسم و اللقب</label>
              ...
              <div>
                <label htmlFor="phone" className="block mb-1">رقم هاتف</label>
                <input type="tel" id="phone" name="phone" required className="w-full px-3 py-2 border rounded" />
              </div>
              <div>
                <label htmlFor="address" className="block mb-1">الولاية و البلدية</label>
                <textarea id="address" name="address" required className="w-full px-3 py-2 border rounded"></textarea>
              </div>
              <div>
                <label htmlFor="delivery" className="block mb-1">اختيار التوصيل</label>
                <select id="delivery" name="delivery" className="w-full px-3 py-2 border rounded">
                  <option value="standard">داخل باتنة</option>
                  <option value="express">الولايات الاخرى</option>
                </select>
              </div>
